...
public class LargeBackpackData {
    ...
    public LargeBackpackData(UUID uuid) {
        this.uuid = uuid;

        this.inventory = new BackpackItemHandler(27);
        this.optional = LazyOptional.of(() -> this.inventory);
    }

    public LargeBackpackData(UUID uuid, CompoundTag incomingNBT) {
        this.uuid = uuid;
    ...
    public static Optional<LargeBackpackData> fromNBT(CompoundTag nbt) {
        if (nbt.contains("UUID")) {
            UUID uuid = nbt.getUUID("UUID");
            return Optional.of(new LargeBackpackData(uuid, nbt));
        }
