...
public class SmallBackpackData {
    ...
    public SmallBackpackData(UUID uuid) {
        this.uuid = uuid;
    ...
    public SmallBackpackData(UUID uuid, CompoundTag incomingNBT) {
        this.uuid = uuid;

        this.inventory = new BackpackItemHandler(9);

        if (incomingNBT.getCompound("Inventory").contains("Size")) {
            if (incomingNBT.getCompound("Inventory").getInt("Size") != 9)
                incomingNBT.getCompound("Inventory").putInt("Size", 9);
        }
    ...
    public static Optional<SmallBackpackData> fromNBT(CompoundTag nbt) {
        if (nbt.contains("UUID")) {
            UUID uuid = nbt.getUUID("UUID");
            return Optional.of(new SmallBackpackData(uuid, nbt));
        }
